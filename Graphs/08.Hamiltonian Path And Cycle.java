...
public class Main {
    ...
    public static void printHamiltonion(ArrayList < Edge > [] graph, int src, HashSet < Integer > visited, String psf, int osc) {
        if (visited.size() == graph.length - 1) {
            boolean check = false;
            for (Edge e: graph[osc]) {
                if (e.nbr == src) {
                    check = true;
                    break;
                }
        ...
        for (Edge e: graph[src]) {
            if (visited.contains(e.nbr) == false) {
                printHamiltonion(graph, e.nbr, visited, psf + e.nbr, osc);
            }
        }
        visited.remove(src);
    }
